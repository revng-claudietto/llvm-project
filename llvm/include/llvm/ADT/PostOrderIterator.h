...
public:
  ...
  // When GraphT is already a NodeRef (i.e. the caller starts the traversal
  // from a specific node, leveraging the NodeRef-as-graph pattern) use it
  // directly, so that an explicitly-supplied GT — which may not provide a
  // getEntryNode overload for GraphT, e.g. GraphTraits<Inverse<...>> at a
  // different graph level — is honored.
...
class ReversePostOrderTraversal {
  ...
  void Initialize(const GraphT &G) {
    // Use po_iterator directly with the explicit GT, otherwise the deduced
    // po_begin/po_end would silently pick GraphTraits<GraphT> and ignore an
    // explicitly-supplied GT (e.g. GraphTraits<Inverse<...>>).
